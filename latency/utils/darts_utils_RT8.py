...
def build_engine(model_file):
    with trt.Builder(TRT_LOGGER) as builder:
        ...
        with builder.create_builder_config() as config, builder.create_network(explicit_batch) as network, trt.OnnxParser(network, TRT_LOGGER) as parser:
...
def compute_latency_ms_tensorrt(model, input_size, iterations=None):
    ...
    torch.onnx.export(model, dummy_input, "model.onnx", verbose=True, input_names=["input"], output_names=["output"], export_params=True,opset_version=11)
    ...
    with build_engine("model_sim.onnx") as engine:

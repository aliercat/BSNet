...
from nets.stem_sesp_stdcnet import STDCNet1446, STDCNet813
...
class BiSeNet(nn.Module):
    def __init__(self, backbone, n_classes, pretrain_model='', use_boundary_2=False, use_boundary_4=False, use_boundary_8=False, use_boundary_16=False, use_conv_last=False, heat_map=False, *args, **kwargs):
        ...
        self.aux4 = SegmentHead(512, 128, n_classes, up_factor=16, aux=False)
        self.aux5 = SegmentHead(1024, 128, n_classes, up_factor=32, aux=False)
...
#dd
    def forward(self, x):
        ...
        #融合结构得到结果
        feat_bga = self.bga(feat_res8,feat32)
        #分割头处理，恢复原大小，并得到19种分类概率
        logits = self.head(feat_bga)
        # print()
        # print(logits.shape)

        logits_aux4 =self.aux4(feat16)
        logits_aux5 =self.aux5(feat32)
        ...
        if (not self.use_boundary_2) and (not self.use_boundary_4) and self.use_boundary_8:
            return logits,logits_aux4,logits_aux5, feat_out_sp8
        
        if (not self.use_boundary_2) and (not self.use_boundary_4) and (not self.use_boundary_8):
            return logits,logits_aux4,logits_aux5

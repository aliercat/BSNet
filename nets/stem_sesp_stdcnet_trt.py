...
class SELayer(nn.Module):
    ...
    def forward(self, x):
        ...
        #stridesz = np.floor(inputsz/outputsz).astype(np.int64) 
        ...
        #RT
        return x * y.expand(-1,-1,x.shape[2],x.shape[3])
...
class SPBlock(nn.Module):
    def __init__(self, inplanes, outplanes, norm_layer=nn.BatchNorm2d):
        ...
        # self.pool1 = nn.AvgPool2d((None, 1))
        # self.pool2 = nn.AvgPool2d((1, None))
    ...
    def forward(self, x):
        ...
        #inputsz1 = np.array(x.shape[2:])
        outputsz1 = np.array([h,1])
        
        stridesz1 = np.floor(inputsz/outputsz1).astype(np.int32) 
        kernelsz1 = inputsz-(outputsz1-1)*stridesz1

        avg1=nn.AvgPool2d(kernel_size=list(kernelsz1),stride=list(stridesz1))
        x1 =avg1(x)
        ...
        #inputsz2 = np.array(x.shape[2:])
        outputsz2 = np.array([1,w])
        
        stridesz2 = np.floor(inputsz/outputsz2).astype(np.int32) 
        kernelsz2 = inputsz-(outputsz2-1)*stridesz2

        avg2 =nn.AvgPool2d(kernel_size=list(kernelsz2),stride=list(stridesz2))
        x2 =avg2(x)
        #x2 = self.pool2(x)
        x2 = self.conv2(x2)
        x2 = self.bn2(x2)
...
class STDCNet1446(nn.Module):
    def __init__(self, base=64, layers=[4,5,3], block_num=4, type="sesp", num_classes=1000, dropout=0.20, pretrain_model='', use_conv_last=False):
        ...
        #self.x8 = nn.Sequential(self.features[:5])
...
class STDCNet813(nn.Module):
    def __init__(self, base=64, layers=[2,2,2], block_num=4, type="sesp", num_classes=1000, dropout=0.20, pretrain_model='', use_conv_last=False):
...
if __name__ == "__main__":
    model = STDCNet813(num_classes=1000, dropout=0.00, pretrain_model='/home/fzl/dfr/my_improved_STDC_copy3/checkpoints/STDCNet813M_73.91.tar', block_num=4)

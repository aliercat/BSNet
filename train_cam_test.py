...
def train():
    ...
    dspth = '/home/test/dfr/my_improved_STDC_sesp_BGA_DH_cam/data/CamVid/'

    train_path = os.path.join(dspth,"train")
    val_path = os.path.join(dspth,"test")
    train_labels_path = os.path.join(dspth,"train_labels")
    val_labels_path = os.path.join(dspth,"test_labels")
    class_dict_path = os.path.join(dspth,"class_dict.csv")
    ...
    dist.init_process_group(
                backend = 'nccl',
                init_method = 'tcp://127.0.0.1:33319',
    ...
    mode = args.mode
    cropsize = [960, 704]
    randomscale = (0.5, 1, 1.25, 1.5, 1.75, 2, 2.25, 2.5)
    ...
    ds = CamVid([train_path],
                  [train_labels_path],class_dict_path,
                  cropsize=cropsize, randomscale=randomscale,mode='train')
    ...
    dsval = CamVid([val_path],
                  [val_labels_path],class_dict_path,
                  cropsize=cropsize, randomscale=randomscale,mode='test')
    ...
    for it in range(max_iter):
        try:
            im, lb = next(diter)
            if not im.size()[0]==n_img_per_gpu: #不够一个bs了说明是一个epoch到了
                raise StopIteration
        ...
        if use_boundary_2 and use_boundary_4 and use_boundary_8:
            out, detail2, detail4, detail8 = net(im)
        
        if (not use_boundary_2) and use_boundary_4 and use_boundary_8:
            out, detail4, detail8 = net(im)
        ...
        loss = lossp + boundery_bce_loss + boundery_dice_loss
        ...
        if (it+1)%save_iter_sep==0:# and it != 0:
            ...
            with torch.no_grad():
                ...
                mIOU1 = single_scale2(net, dlval, n_classes)

            # 30000才开始存，1000存一次
            if it+1>40000 and (it+1)%1200==0:
                save_pth = osp.join(save_pth_path, 'model_iter{}_mIOU1_{}.pth'
                .format(it+1, str(round(mIOU1,4))))
            ...
            if mIOU1 > maxmIOU1:
                maxmIOU1 = mIOU1
                save_pth = osp.join(save_pth_path, 'model_maxmIOU1.pth'.format(it+1))
                state = net.module.state_dict() if hasattr(net, 'module') else net.state_dict()
                if dist.get_rank()==0: 
                    torch.save(state, save_pth)
            ...
            logger.info('mIOU1 is: {}'.format(mIOU1))
            logger.info('maxmIOU1 is: {}.'.format(maxmIOU1))
